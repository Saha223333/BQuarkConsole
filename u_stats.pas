...
For j:=3 to q do
  Begin  //получаем имена шлюзов
	i:=Q_PosStr('Шлюз',Main.AutoAnalizerMemo.Lines.Strings[j],0);
	k:=Q_PosStr('недоступен',Main.AutoAnalizerMemo.Lines.Strings[j],0);    
	GateName:=Copy(Main.AutoAnalizerMemo.Lines.Strings[j],i+5,k-7);
   //если шлюз в Березовском, то наращиваем кол-во неопрошенных в березовском
	If Q_PosStr('Березовский',GateName,0)>0 then inc(qb);
  End;
...
If City='SKEK' then
 ...
 OQStats.Close;
 //кол-во шкафов Кемерово за вычетом березовского
 OQStats.SQL.Clear;
 OQStats.SQL.Add('select id_con,con_type from bqrk.con_points '+
 'minus '+
 'select id_con,con_type from bqrk.con_points where dist like ''%Бер%'' ');
 OQStats.Open;
 ...
 If OQStats.Active=true then
 OQStats.Close;
 //кол-во модемов Кемерово за вычетом Березовского
 OQStats.SQL.Clear;
 ...
 If OQStats.Active=true then
 OQStats.Close;
 //кол-во шкафов Березовский
 OQStats.SQL.Clear;
 OQStats.SQL.Add('select id_con,con_type from bqrk.con_points where dist like ''Бер%'' ');
 OQStats.Open;
 ...
 If OQStats.Active=true then
 OQStats.Close;
 //кол-во модемов Березовский
 OQStats.SQL.Clear;
 OQStats.SQL.Add('select id_con,con_type from bqrk.con_points where dist like ''Бер%'' and con_type=1001');
 OQStats.Open;

 A7Rep.SetValue('#BER_MODEMQ#',IntToStr(OQStats.RecordCount));
 PB.IncPartsByOne;
 //делаем запросы по порядку и пишем их в грид
 //сначала все что есть в АИИС
 //Кемерово
 ...
 QryString:='select * from counters where actual=1';
  ...
  PB.IncPartsByOne;
  //----------Березовский прочие
  QryString:='select * from counters where actual=1 and dist like ''Бер%'''; 
 ...
 //-----------Лесная поляна
  ...
  PB.IncPartsByOne;
  //----------------------------------------------------------
  //----------------теперь то что опросе с указанной глубиной
  //----------------------------------------------------------
  //Кемерово
  QryString:=
...
''' minus '
 ...
 +'and date_last_pok>='''+DepthDate.Text+'''';
  ...
  //------------Березовский прочие
   QryString:=
  'select * from counters where actual=1 and dist like ''Бер%'' '
  +'and date_last_pok>='''+DepthDate.Text+'''';
 ...
 'select * from counters where actual=2 and date_last_pok>='''+DepthDate.Text+'''';
  ...
  //кол-во неопрошенных шкафов Кемерово за вычетом Березовского и загаловка
  A7Rep.SetValue('#SHKAFOFF_KEM#',IntToStr(q-4-qb));
  //кол-во неопрошенных шкафов в Березовском
...
If City='LK' then
 ...
 //в целом по городу
  ...
  //кол-во неопрошенных шкафов в Ленинске
